/**************
 * 
 * file : posix_producer_consumer.c
 * 
...
int main(int argc, char const *argv[])
{
    ...
    uint32_t PRODUCERS = (NBTHREADS % 2 == 0) ? NBTHREADS/2 : (NBTHREADS/2) + 1;
    uint32_t CONSUMERS = NBTHREADS/2;
    signed long * array[8];
    ...
    err = pthread_mutex_init(&buffer_mutex, NULL) + pthread_mutex_init(&mutex, NULL);
    if (err != 0)
        error(err, "pthread_mutex_init");

    err = sem_init(&empty, 0, 8) + sem_init(&full, 0, 0);
    if (err != 0)
        error(err, "sem_init");
